...
class TestUserController extends Controller
{
	...
	public function index()
	{
		...
		$results = TestUser::searchTestUsers($search);
	...
	{
		if( !is_null($id) && !($data = TestUser::find($id)) )
		{
			\Flash::error('無効なデータが指定されました。');
			return redirect('user/search');
		}
	...
	{
		// 自身のIDは削除不可
		if( !($TestUser = TestUser::find($id)) || \Auth::guard('user')->user()->id === (int)$id )
		{
			\Flash::error('無効なデータが指定されました。');
			return redirect('user/search');
		}
	...
	{
		if( !($TestUser = TestUser::withTrashed()->find($id)) )
		{
			\Flash::error('無効なデータが指定されました。');
			return redirect('user/search');
		}
